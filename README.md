...
- 伺服器（`server.js`）只作為中繼與加密字串儲存（`db.json`），不會解密訊息。
...
3. 伺服器行為（`server.js`）：
 - 將 `encryptedMsg`以json附加到 `db.json`（作為歷史紀錄）。
...
- 儲存風險：`db.json` 儲存加密字串；若金鑰外洩，歷史訊息可被解密。
...
- The server (`server.js`) acts only as a relay and stores encrypted strings in `db.json`; it does not decrypt messages.
...
3. Server behavior (`server.js`):
 - Append `encryptedMsg` as a json db to `db.json` (history log).
...
- API: `GET /messages` 
 - The server reads `db.json`, splits lines, and returns JSON: 
 `{ "messages": [ "encryptedLine1", "encryptedLine2", ... ] }` 
...
- Storage risk: `db.json` stores encrypted strings—if keys are leaked, historical messages can be decrypted.
